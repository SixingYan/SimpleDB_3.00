 ...
 */
public class KdTreeDir {
   	private TableInfo ti;
   	private Transaction tx;
   	private String filename;
   	private KdTreePage contents;
   	private int level = 0;
    /**
     ...
     */
  	public KdTreeDir(Block blk, TableInfo ti, Transaction tx) {
      	this.ti = ti;
      	this.tx = tx;
      	filename = blk.fileName();
      	contents = new KdTreePage(blk, ti, tx);
   	}
   ...
   public int search(Constant searchkey) {
      	...
      	while (contents.getFlag() > 0) { // 如果当前的数据没存储完，说明还有下一个block存储这个值，于是就关了当前的，继续打开下一个
         	contents.close();
         	contents = new KdTreePage(childblk, ti, tx);
