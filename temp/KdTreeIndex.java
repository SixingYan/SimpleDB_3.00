 ...
 * @author Sixing Yan
 */
public class KdTreeIndex implements Index {
	public static int MAX_STRING = 256;
	private Transaction tx;
	private TableInfo dirTi, leafTi;
	private KdTreeLeaf leaf = null;
	...
	*/
	public KdTreeIndex(String idxname, Schema leafsch, Transaction tx) {
		...
		if (tx.size(dirTi.fileName()) == 0)
			...
			tx.append(dirTi.fileName(), new KdPageFormatter(dirTi, 0)); // 他用的是和leafTi一样的page
		KdTreePage page = new KdTreePage(rootblk, dirTi, tx);
		if (page.getNumRecs() == 0)
			page.insertDir(0, new Constant(DOUBLE.MIN_VALUE), 0, 0);
	  	page.close();
   }
   ...
   public void beforeFirst(Constant searchkey) {
	  close();
	  KdTreeDir root = new KdTreeDir(rootblk, dirTi, tx);
	  int blknum = root.search(searchkey); //因为是从头开始的，所以都是从 0 level 开始
	  root.close();
